...
async function GetListings(user){
    let url = `https://api.mercadolibre.com/users/${user.user_id}/items/search?access_token=${user.access_token}`;
...
async function GetListing(listingId, user){
    let url = `https://api.mercadolibre.com/items/${listingId}?access_token=${user.access_token}`;
    let results = await axios.get(url)
    .then((res) => {
        return res.data;
    })
